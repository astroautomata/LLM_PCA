...
for pca_i in "${PCA_COMPS_I[@]}"; do
    for pca_o in "${PCA_COMPS_O[@]}"; do
        ...
        # Run the experiment
        python3 experiment5.py $pca_i $pca_o --max-chars $MAX_CHARS
        EXIT_CODE=$?
...
echo "=================================="
echo "Generated results in: experiment5/"
find experiment5 -type f -name "experimental_results.json" -exec ls -lh {} \; 2>/dev/null || echo "No result files found"
